...
import { View, Text, StyleSheet, TouchableOpacity, FlatList, SafeAreaView, Image, TextInput } from 'react-native';
import { AntDesign, FontAwesome, MaterialCommunityIcons } from '@expo/vector-icons'; // For icons
...
const Report = () => {
  ...
  const renderListItem = (item: string | number | boolean | React.ReactElement<any, string | React.JSXElementConstructor<any>> | Iterable<React.ReactNode> | null | undefined) => (
    <TouchableOpacity style={styles.listItem} onPress={() => handleItemSelect(item as string)}>
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.tabContainer}>
        {/* Tab Buttons */}
        <View style={[styles.tabButton, selectedTab === 0 && styles.tabButtonActive]}>
          <FontAwesome name="check-circle" size={16} color={selectedTab === 0 ? '#00C853' : '#BDBDBD'}/>
          <Text style={styles.tabText}>Select Issue</Text>
          <Text style={styles.tabText}>Type</Text>
        </View>
        <View style={[styles.tabButton, selectedTab === 1 && styles.tabButtonActive]}>
          <FontAwesome name="check-circle" size={16} color={selectedTab === 1 ? '#00C853' : '#BDBDBD'}/>
          <Text style={styles.tabText}>Select Question</Text>
          <Text style={styles.tabText}>Type</Text>
        </View>
        <View style={[styles.tabButton, selectedTab === 2 && styles.tabButtonActive]}>
          <FontAwesome name="check-circle" size={16} color={selectedTab === 2 ? '#00C853' : '#BDBDBD'}/>
      ...
      <View style={styles.contentContainer}>
        {selectedTab === 0 && (
          <FlatList data={issueTypes} renderItem={({ item }) => renderListItem(item)} keyExtractor={(item) => item} />
        )}
        {selectedTab === 1 && (
          <FlatList data={questionTypes} renderItem={({ item }) => renderListItem(item)} keyExtractor={(item) => item} />
        )}
        {selectedTab === 2 && (
          // Custom content for the Feedback tab
          <View>
           <Text style={styles.label}>Transaction Number <MaterialCommunityIcons size={10} name='asterisk' color={'red'} /></Text>
           <TextInput
           placeholder='627565265OTJH0HF'
           style={styles.input}
            />

            <Text style={styles.label}>Upload Photo <MaterialCommunityIcons size={10} name='asterisk' color={'red'} /></Text>
            ...
            <View style={styles.noteContainer}>
                <AntDesign name='exclamationcircleo' size={15} color={'#0000ff'}/>
                <Text style={styles.note}>Note: not more than 3 photos. JPEG, PNG 5MB Max.</Text>
            </View>
            
            <Text style={styles.label}>Description <MaterialCommunityIcons size={10} name='asterisk' color={'red'} /> </Text>
            <TextInput
      ...
      <BottomSheet isVisible={isSuccessVisible} onBackdropPress={() => setIsSuccessVisible(false)}>
        <View style={styles.bottomSheetContent}>
          ...
          <Text style={styles.successBottomSheetHeader}>Report Sent Successfully</Text>
      <Text style={styles.desc}>Your report has been submitted successfully and under review.</Text>

      <TouchableOpacity style={styles.nextButton} onPress={() => router.dismissAll()}>
