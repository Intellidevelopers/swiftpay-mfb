...
const Payback = () => {
  const totalPayment = missedPayments.reduce((sum, item) => sum + parseInt(item.amount.replace('-', '').replace('$', '')), 0);

  const renderPaymentItem = ({ item }: { item: { id: string; date: string; amount: string } }) => (
