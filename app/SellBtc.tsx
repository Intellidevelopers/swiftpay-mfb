...
const SellBtc = () => { 
 ...
 const { walletAddress, network } = useWalletStore(); // Retrieve wallet details
  ...
  const [usdtSellAmount, setUsdtSellAmount] = useState(''); // State to store USDT sell amount
  const sellExchangeRate = 740; // Conversion rate for selling USDT to NGN

  // Calculate amount in NGN
  const amountInNGN = usdtSellAmount ? (parseFloat(usdtSellAmount) * sellExchangeRate).toFixed(2) : '0.00';
  ...
  return (
    <SafeAreaProvider>
      ...
      <Text style={styles.paymentText}>Amount of USDT to Sell</Text>
      <TextInput 
        style={styles.input} 
      ...
      <View style={styles.estimate}>
        <Text style={styles.estimateValue}>{amountInNGN} NGN</Text>
          ...
          <Text style={styles.noteBottom}>
            The coin you Sell will be sent to the wallet address above.
          </Text>
        </ScrollView>
        ...
        <BottomSheet isVisible={isPreviewVisible} onBackdropPress={() => setIsPreviewVisible(false)}>
          <View style={styles.bottomSheetContent}>
            ...
            <TouchableOpacity
             style={styles.SellButton} onPress={handleContinue}>
